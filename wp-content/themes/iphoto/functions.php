...
if ( !function_exists('pagenavi') ) {
	function pagenavi( $p = 3 ) { // 取当前页前后各 2 页
		...
		for( $i = $paged - $p; $i <= $paged + $p; $i++ ) { // 中间页
			if ( $i > 0 && $i <= $max_page ) $i == $paged ? print "<span class='page-numbers' id='post-current' title='{$max_page}'>{$i}</span> " : p_link( $i );
		}
		...
		<div id="theme-content">
			<ul>
				...
				<tr><td>
					...
					<label><input name="noajax" type="checkbox" id="noajax" value="1" <?php if (get_option('iphoto_noajax')!='') echo 'checked="checked"' ;?>/><?php _e( 'Close the Infinite loading posts',THEME_NAME);?></label><br/><br/>
				</td></tr>
				<tr><td>
