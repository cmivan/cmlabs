...
<div class="wrap">
<div>
<h2><?php _e( '图格设置<span>Version: ',THEME_NAME);?><?php $theme_data=get_theme_data(TEMPLATEPATH . '/style.css'); echo $theme_data['Version'];?></span></h2>
</div>
<div class="clear"></div>
<form method="post" action="">
	<div id="theme-Option">
		<div id="theme-menu">
			<span class="m1"><?php _e( 'jQuery',THEME_NAME);?></span>
			<span class="m4"><?php _e( '统计代码',THEME_NAME);?></span>
			<span class="m5"><?php _e( '页面版权',THEME_NAME);?></span>
			<div class="clear"></div>
		</div>
		<div id="theme-content">
			<ul>
				<li>
				<tr><td>
					<em><?php _e( '图格默认是使用 jquery 1.4.4 的, 但你也可以直接使用 Google 的.',THEME_NAME);?></em><br/>
					<label><input name="lib" type="checkbox" id="lib" value="1" <?php if (get_option('iphoto_lib')!='') echo 'checked="checked"' ;?>/><?php _e( '从Google加载 jQuery',THEME_NAME);?></label><br/><br/>
				</td></tr>
				<tr><td>
					<em><?php _e( '图格页面是否使用无限加载？.',THEME_NAME);?></em><br/>
					<label><input name="noajax" type="checkbox" id="noajax" value="1" <?php if (get_option('iphoto_noajax')!='') echo 'checked="checked"' ;?>/><?php _e( '关闭无限加载',THEME_NAME);?></label><br/><br/>
				</td></tr>
				<tr><td>
					<em><?php _e( '<strong>图格布局的动画效果</strong>',THEME_NAME);?></em><br />
					<input name="animate" type="checkbox" id="animate" value="1" <?php if (get_option('iphoto_animate')!='') echo 'checked="checked"';?>/><?php _e( '激活动画效果!, 选择相应的效果',THEME_NAME);?>
				...
				<li>
                <tr><td>
				<?php _e( '你可以把你的Google统计代码放在这里呵 <a target="_blank" href="https://www.google.com/analytics/settings/check_status_profile_handler">就是这里</a>.',THEME_NAME);?></label><br>
  ...
  <tr><td>
      ...
      <br/><em><?php _e( '<b>效果</b>',THEME_NAME);?><span> : </span><span><?php if(stripslashes(get_option('iphoto_copyright'))!=''){echo stripslashes(get_option('iphoto_copyright'));}else{echo 'Copyright &copy; '.date('Y').' '.'<a href="'.home_url( '/').'" title="'.esc_attr( get_bloginfo( 'name') ).'">'.esc_attr( get_bloginfo( 'name') ).'</a> All rights reserved'; };?></span></em>
  </td></tr>
				</li>
			</ul>
		</div>
