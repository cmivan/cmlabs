...
<div id="cate" title="<?php echo get_option('iphoto_easing');?>" alt="<?php echo get_option('iphoto_animate');?>" noajax="<?php echo get_option('iphoto_noajax');?>"><?php $category=get_the_category($post->ID);$name = $category[0]->slug;echo $name;?></div>
<!--end cate-->

<script type="text/javascript" src="<?php bloginfo('template_url'); ?>/javascript/jquery.waterfall.min.js"></script>
...
<div id="phone_waterfall_phone_left">
    <?php if(have_posts()) : while (have_posts()) : the_post(); ?>
        <?php get_template_part( 'iphoto_content', get_post_format() ); ?>
    <?php endwhile; endif; ?>
</div>
</div>
